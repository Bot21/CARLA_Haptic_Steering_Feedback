...
# 'rev_part_block', 
OBSTACLE_FORMATION = 'for_part_block'
...
safe_distance = 5 #A buffer distance the Ego must maintain with obstacles
...
IDEAL_LANECHANGE_DISTANCE = 20 #The ideal distance ahead of the first obstacle where the Ego returns to the orginal lane
...
class Vehicle():
    ...
    def init_controller(self):
        self.vehicle_controller = ModVehiclePIDController(self.vehicle,
        ...
        self.target_waypoint = current_waypoint.transform
    ...
    def run_step(self, debug=False):
        ...
        for waypoint in self._waypoints_queue:
            ...
            if veh_location.distance(waypoint.location) < min_distance:
        ...
        #if debug:
            #draw_waypoints(self.vehicle.get_world(), [self.target_waypoint], 1.0)
...
def spawn_spec(world, ego_vehicle):
    ...
    spec_spawn_point.location.z = spec_spawn_point.location.z + 30
    # spec_spawn_point.location.y = spec_spawn_point.location.y - 2
    # spec_spawn_point.location.x = spec_spawn_point.location.x + 63.75
    # spec_spawn_point.rotation.pitch = spec_spawn_point.rotation.pitch - 90
    # spec_spawn_point.rotation.yaw = spec_spawn_point.rotation.yaw - 90
...
def shortlist_obstacles(wmap, event):
    ...
    global obstacle_list
    #Compile a list of obstacles in the same road (not lane) as Ego
    obs_list = []
...
def verify_obs(ego, wmap, obs_list):
    ...
    #Number of waypoints to check ahead for obstacle
    wp_num = 10
...
def find_destination(wmap, obs_start, obs_list):
    ...
    #to find a viable waypoint, in the same lane that is unoccupied by obstacles
    wp = obs_waypoint.next(sample_resolution)[0]
    while not empty_space_found and wp:
        for obs_wp in obstacle_list:
            obs_loc = obs_wp.transform.location
            #If too close to any one obstacle, it fails
            wp_to_obs = wp.transform.location.distance(obs_loc)
            if wp_to_obs < DEST_CLEARANCE:
                empty_space_found = False
                next_wp = wp.next(sample_resolution)[0]
                wp = next_wp
                break
    ...
    #Save the waypoint location of the first viable waypoint in first_wp_loc
    first_wp_loc = wp.transform.location

    #Now keep searching through viable waypoints until another obstalce is encountered,
    #OR it is far enough from the first viable waypoint
    #OR it has reached the end of the current road, by check if it has hit a traffic junction
    while not destination_found and wp:
        for obs_wp in obstacle_list:
            obs_loc = obs_wp.transform.location
            #If too close to any one obstacle, it fails
            wp_to_obs = wp.transform.location.distance(obs_loc)
            wp_to_first_wp = wp.transform.location.distance(first_wp_loc)
            if (wp_to_obs < DEST_CLEARANCE \
                or wp_to_first_wp > IDEAL_LANECHANGE_DISTANCE \
                or wp.is_junction):
...
def select_waypoints(ego_waypoint, obs_wp, wmap, destination, obs_list):
    ...
    if left_wps == []: #and right_wps == []
        ...
        #Construct a path that begins at the Ego and ends at the destination
        route.append(ego_waypoint)

...
max_obstacles = 50 # Number of obstacles that will try to spawn (limit is about 20-25)
...
min_distance = 20 #minimum distance a spawned vehicle must be from road junctions and other vehicles
...
waypoint_queue = deque(maxlen = 1000) #list of waypoints behind front vehicle for back vehicle to follow
...
def back_control_update(world, frontvehicle, backvehicle, back_controller, logi_controller, changing_lane):
    ...
    front_waypoint = world.get_map().get_waypoint(frontvehicle.get_location()) 
    front_speed = get_speed(frontvehicle)

    global waypoint_queue

    waypoint_queue.append(front_waypoint)
    ...
    for waypoint in waypoint_queue:
        ...
        if len(waypoint_queue) - num_waypoint_removed == 1:
    ...
    if len(waypoint_queue) == 0:
        ...
        target_waypoint = waypoint_queue[0]
        back_location = backvehicle.get_location()
        ...
        #Sets max speed to front vehicle's instantaneous speed + 2 
        setpoint_speed = min(desired_speed, front_speed + 2)
...
def lanechanging(ego, custom_controller, world, sensor, event):
    ...
    if obs_check == True:
        ...
        #Unsubscribe from Traffic Manager
        #Fetch location of nearby obstacles
        #Fetch all nearby waypoints
        #Eliminate occupied waypoints
        #Identify destination waypoint from remaining list

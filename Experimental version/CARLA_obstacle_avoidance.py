...
NUM_OF_OBSTACLES = 50 #Number of obstacles to be spawned on the map
...
def spawn_obstacle_rand(world):
    ...
    if spawn_len > 0:
        ...
        spawn = spawn_list[rand_int]

        obs = Obstacle(world, spawn)
...
def destroy_actors(client, world): 
    ...
    global actor_list
    global obstacle_list
    if client.apply_batch([carla.command.DestroyActor(x) for x in actor_list]):
        actor_list = []
    if client.apply_batch([carla.command.DestroyActor(x) for x in obstacle_list]):
        obstacle_list = []
